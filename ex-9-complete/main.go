...
func handleRequests() {
	...
	//virtual workshop ex-9 solution, register command and event handler here
	bus.RegisterCommandHandler(&commands.CancelReservation{}, commandsHandler)
	bus.RegisterEventHandler(&hotelevents.ReservationCanceled{}, eventsHandler)
	...
	//virtual workshop ex-9 solution, add another http.HandleFunc for the /cancelReservation route

	http.HandleFunc("/cancelReservation", func(w http.ResponseWriter, r *http.Request) {
		handlers.CancelReservation(w, r, *bus, inventoryByRoomType, db)
	})
