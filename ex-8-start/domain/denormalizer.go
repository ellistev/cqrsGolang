...
func deleteReservation(db *sql.DB, id uuid.UUID) *Reservation{
	log.Println("Inserting reservation record ...")
	selectReservationSQL := `DELETE FROM reservation where reservationId = "` + id.String() + "\""
	_, err := db.Exec(selectReservationSQL)
	if err != nil {
		log.Fatal(err)
	}
	return nil
...
//virtual workshop ex-8 implement an event handler here for onReservationCanceled
//the above method will handle the actual deleting of a reservation from the readmodels, so that's already done for you :)
//this should be simliar to OnReservationMade but a little simpler as we're just removing a record on cancel